...
export default function TraderStockPage({ overrideSymbol, onBack }) {
  ...
  const displayChangePercent = (liveChangePercent !== undefined && liveChangePercent !== null) ? liveChangePercent : (stock.changePercent ?? 0);
  const displayChangeAmount = (liveChange !== undefined && liveChange !== null) ? liveChange : (stock.changeAmount ?? 0);
  ...
  const loadTerminalData = async (isSilent = false) => {
    ...
    try {
      const [aggRes, generalNewsRes, finnhubNews] = await Promise.all([
        api.get('/stock/' + encodeURIComponent(symbol)).catch(() => null),
      ...
      const price = Number(dbQuote.price || dbQuote.current || 0) || (livePrice || 0);
      const changePercent = Number(dbQuote.changePercent ?? dbQuote.change ?? 0);
      const changeAmount = price > 0 && Math.abs(changePercent) > 0
        ? price * changePercent / 100
        : 0;

      const openPrice = Number(dbQuote.open) || price;
      const highPrice = Number(dbQuote.high) || price;
      const lowPrice  = Number(dbQuote.low)  || price;
      const prevClose = Number(dbQuote.previousClose || dbQuote.prevClose) || price;

      setStock({
        symbol: symbol.replace(/\.(NS|BO)$/i, ''),
        name: dbQuote.name || companyProfile.name || symbol,
        price,
        changePercent,
        changeAmount,
        volume: dbQuote.volume != null && Number(dbQuote.volume) > 0
          ? (isCryptoAsset
              ? String(Number(dbQuote.volume).toLocaleString()) + ' ' + am.currency
              : (Number(dbQuote.volume) / 1e6).toFixed(2) + 'M')
          : '--',
      ...
      setTechData({
        rsi:  tech.rsi  ?? null,
        macd: tech.macd ?? { hist: null, signal: 'Neutral' },
        vwap: tech.vwap ?? (price > 0 ? price * 1.002 : null),
        atr:  tech.atr  ?? null,
        emas: tech.emas ?? {
          ema20: price > 0 ? price * 0.995 : null,
          ema50: price > 0 ? price * 0.980 : null,
          cross: 'Neutral',
        },
        rvol:       tech.rvol       ?? null,
      ...
      const ratios = fundamentalsData.ratios || {};
      setStockDetails({
        ...companyProfile,
        ...dbQuote,
        open:          openPrice,
        ...
        peRatio:       fundamentalsData.peRatio ?? ratios.peRatio ?? null,
        pbRatio:       ratios.pbRatio       ?? null,
        evEbitda:      ratios.enterpriseValueToEbitda ?? null,
        roe:           ratios.roe           ?? null,
        roa:           ratios.roa           ?? null,
        ...
        longBusinessSummary: companyProfile.summary || companyProfile.longBusinessSummary || null,
        website:  companyProfile.website  || null,
        avgVolume:        dbQuote.volume    ?? null,
        marketCap:        dbQuote.marketCap ?? null,
        fiftyTwoWeekLow:  fundamentalsData.fiftyTwoWeekLow  ?? dbQuote.fiftyTwoWeekLow  ?? null,
        fiftyTwoWeekHigh: fundamentalsData.fiftyTwoWeekHigh ?? dbQuote.fiftyTwoWeekHigh ?? null,
        beta:             dbQuote.beta      ?? null,
      });

      if (companyProfile.website) {
        const domain = companyProfile.website.replace(/https?:\/\/(www\.)?/, '').split('/')[0];
      ...
      if (finnhubNews && finnhubNews.length > 0) {
        ...
        setNews(rawGeneralNews.map((a, i) => ({
          ...
          url:       a.url       || '#',
          image:     a.image     || null,
          summary:   a.summary   || a.description || 'Read market briefings on major exchanges.',
        })));
      }
